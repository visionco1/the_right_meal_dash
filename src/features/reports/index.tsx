...
import ExportFinance from './export-fnance'
            ...
            <CardHeader>
                {permission('show') && (
                    <div className='flex gap-2 flex-wrap'>
                        ...
                        <ExportFinance date={filter_data?.date} />
                    </div>
                )}
            </CardHeader>
            <CardBody>
                    ...
                    <>
                        <div className='filter'>
                            ...
                            <div className='grid grid-cols-2 lg:grid-cols-4 gap-3 items-end'>
                                ...
                                <FormInput
                                    onChange={(e: any) =>
                                        handleChangeFilter('date', e.target.value)
                                    }
                                    label={t('date')}
                        ...
                        <Table className='mt-8 w-full print:block' ref={tableRef}>
                            ...
                            <TableBody>
                                {data?.data?.data?.map((item: any, i: number) => (
                                    <TableRow
                                        key={i}
                                        className={`relative ${i % 2 === 0 ? 'bg-gray-50 dark:bg-gray-600' : ''}`}
                                    >
                                        <TableCell>{item?.localized_sub_name}</TableCell>
                                        <TableCell className='text-primary'>
                                            {item?.customer_id ? (
                                                <Link
                                                    to={`/customers-management/customers/${item?.customer_id}`}
                                                >
                                                    {item?.localized_customer_name}
                                                </Link>
                                            ) : (
                                                item?.localized_customer_name
                                            )}
                                        ...
                                        <TableCell>
                                            <p className='h-20 overflow-auto'>
                                                {item?.meals
                                                    ?.map((m: any) => m?.localized_meal_name)
                                                    ?.join(',')}
                                            </p>
                                        </TableCell>
