...
const StaffForm = ({ handleSubmit, handleChange, data, setOpenModal, showResponse }: TProps) => {
			...
			<div className="h-auto">
				<div className="grid grid-cols-1 md:grid-cols-2 justify-between gap-3 items-center">
					...
					<CustomSelect
						...
						label={t('permitted_branches')}
						name="permitted_branches"
						placeholder={t('permitted_branches')}
						isMulti
						options={branches}
						value={setMultiSelect(branches, data?.permitted_branches)}
					...
					<CustomSelect
						onChange={(e: any) => {
							handleChange('role_id', e?.id)
						}}
						label={t('role_id')}
						name="role_id"
						placeholder={t('role_id')}
						options={roles?.data?.data || []}
						value={setSelect(roles?.data?.data || [], data?.role_id)}
						optionLabel="localized_name"
						optionValue="id"
					/>
