...
const UpdateRoleContent = () => {
	...
	const { updateLoading, handleUpdateRole, showLoading, showResponse, handleChangeFrom, data: updatedData } = useUpdateRole()
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 items-center">
				{data?.data[key]?.map((item: any) => (
					<CustomSwitch
						...
						id={item?.id}
						value={item?.id}
						defaultChecked={updatedData?.permissions?.includes(String(item?.id))}
						labelOff={t('off')}
						labelOn={t('on')}
	...
	return (
		<Card>
			<CardBody>
				{(updateLoading || showLoading || loading) && <Loading />}
				{showResponse && !showLoading && (
					<form onSubmit={handleUpdateRole} className="relative">
						<div className="flex justify-between gap-5 items-center">
							<div className="basis-1/2">
								<FormInput defaultValue={showResponse?.data?.name?.ar} onChange={handleChangeFrom} name="display_name:ar" label={t('name_ar')} />
							</div>
							<div className="basis-1/2">
								<FormInput defaultValue={showResponse?.data?.name?.en} onChange={handleChangeFrom} name="display_name:en" label={t('name_en')} />
							</div>
						</div>
						<Tab.Group>
...
export default UpdateRoleContent
