		...
		<>
			<Transition appear show={open} as={Fragment}>
				<Dialog as="div" className="relative" onClose={toggleOffcanvas}>
					...
					>
						<div
							className={`fixed duration-300 overflow-y-auto z-50 transform overflow-hidden card rounded-none shadow-xl transition-all ${sizeClassName ?? ''} ${placement ?? 'end'}-0 ${placement == 'top' || placement == 'bottom' ? 'min-w-full w-full inset-x-0' : 'min-h-full h-full inset-y-0'}`}
						>
