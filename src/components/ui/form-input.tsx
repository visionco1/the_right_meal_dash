...
const Label = ({
    ...
    return (
        <label
            className={cn('font-semibold text-gray-500 capitalize', className)}
            htmlFor={htmlFor}
        >
            {label}
        </label>
...
const PasswordInput = ({
    ...
    return (
        <div className='flex items-center'>
            ...
            />
            <span
                className='px-3 py-1 border rounded-e-md -ms-px dark:border-white/10 cursor-pointer'
    ...
    return (
        <div className={`flex justify-start items-center relative ${className}`}>
            <button
                ...
                className='text-center z-10 w-6 h-6 text-xs font-bold absolute top-0 right-0 bg-gray-900 text-white rounded-full'
            >
                X
            </button>
            <div
                className={`${imgWidth || 'w-24 h-24'} relative bg-gray-200 border-4 border-dashed overflow-hidden border-gray-400 rounded-full`}
            >
                {image ? (
                ...
                <div className={containerClass}>
                    <div className='flex items-center'>
                        <input
                            type={type}
                            {...commonProps}
                            className={`form-checkbox rounded text-primary ${error ? 'border-red-500 text-red-700' : ''}`}
                        />
                        <Label label={label} htmlFor={name} className={labelClassName} />
                    </div>
                </div>
